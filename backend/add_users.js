...
async function addUsers() {
  try {
    ...
    await db.query("DELETE FROM users WHERE email IN ('admin2@example.com', 'owner2@example.com', 'kasir2@example.com')");

    console.log('Menambahkan user baru...');
    
    // Hash password
    const hashedPassword = await bcrypt.hash('admin821', 10);
